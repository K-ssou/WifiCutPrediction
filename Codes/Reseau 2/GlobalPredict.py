...
if len(sys.argv)<3:
	print("------- python GlobalPredict.py ListPhonesFile ALGO={RND, AVG, ADA} ---------")
	sys.exit(0)
...
for h in horizons:
	...
	for imei in phones: 
		...
		print("Compute wifi prediction for ALGO = {}, horizon = {}, training = {} w, phone = {}".format(algo, h, training, imei[0]))
#		if (algo=='RND'):
#		 	Prediction_Wifi_RND.predict(imei[0], h, training, stat_filename, cuts_filename, resumes_filename)
		...
		if (algo=='ADA'):
			...
			Prediction_Wifi_ADA.predict(imei[0], h, training, maxPredictDays, README_filename, sharedWifiList)
