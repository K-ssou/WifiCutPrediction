...
def predict(
    device_id, horizon, TrainingPeriod, maxPredictDays, README_filename, sharedWifiList
):
    ...
    EndNbPredictions = (
        maxPredictDays * 24 * 60 / (horizon * 15)
    )  # interval pour mesures toutes les 5 minutes
    ...
    [idDayWifi, idNightWifi, top] = getHomeWorkWifi(
        db, device_id, UsedWifi, "tmp_file", sharedWifiList
    )
    ...
    cursor_wifi.execute(
        """select timestamp_fire from measurements m where m.device_id='{}'""".format(
            (device_id)
        )
    )
    if cursor_wifi.rowcount > 0:
        print(
            "la requete Measurements a retourne 	{} mesures 	pour imei {}".format(
                cursor_wifi.rowcount, device_id[0:4]
            )
        )
    else:
    ...
    while True:
        ...
        if row is None:
            ...
            if (
                (Type is not None)
                and (cpt > TrainingPeriodNbMes)
                and (date - First_Date > TrainingPeriodWeek)
                and (NbPredictions <= EndNbPredictions)
            ):
                # printTS(ts)
                ...
                if Type is not None:
                    ...
                    if (
                        datetime.strptime(ts2string(row[0]), "%Y-%m-%dT%H:%M:%S")
                        > date_init
                    ):
                        ...
                        for i in range(Nb_Slot):
                            date = ts2string(ts)
                            date = datetime.strptime(date, "%Y-%m-%dT%H:%M:%S")
                            SlotInf = date.replace(
                                hour=0, minute=0, second=0, microsecond=0
                            )

                            if (
                                date >= SlotInf + Slot * i
                                and date < SlotInf + Slot * i + Slot
                            ):
                                # Avoir l'index i du slot auquel correspond la date
                                cur_index = i
                                if DEBUG == True:
                        ...
                        print(
                            "Mesure effectuée avant création de la table connectivity"
                        )
