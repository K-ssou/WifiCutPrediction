...
def getListUsedWifi(db, device_id, DEBUG=False):
    ...
    cursor_scan.execute(
        """select ssid from connectivity c, measurements m where c.measurement_id=m.id and m.device_id='{}' and type=1""".format(
            (device_id)
        )
    )
    ...
    while True:
        ...
        if row is None:
            if cpt == 0:
                if DEBUG:
                    print(
                        "Aucune mesure envoyé pour wifi ou ID mobile non existant dans MACACO SERVER"
                    )
            break
        cpt = cpt + 1
...
def getHomeWorkWifi(db, device_id, UsedWifi, hw_filename, sharedWifiList, DEBUG=False):
    ...
    cursor_scan = db.cursor()
    cursor_scan.execute(
        """select timestamp_fire, w.name from wifi w, measurements m where m.id=w.measurement_id and m.device_id='{}'""".format(
            (device_id)
        )
    )
    if cursor_scan.rowcount > 0:
    ...
    while True:
        ...
        if row is None:
            if cpt == 0:
                if DEBUG:
                    print(
                        "Aucune mesure envoyé pour 3G ou ID mobile non existant dans MACACO SERVER"
                    )
            break
        date_init = datetime.strptime(
            "2015-05-22T00:00:00", "%Y-%m-%dT%H:%M:%S")
        ts = row[0]  # timeslot
        ...
        if date_ts > date_init:
            ...
            if wifi_id >= 0:
                ...
                eightAM = date_ts.replace(
                    hour=8, minute=0, second=0, microsecond=0)
                eightPM = date_ts.replace(
    ...
    hw_file.write(
        "\n \n IMEI {} Home Work diff = {}".format(
            device_id, (idMaxN != idMaxD))
    )
    hw_file.write("\n Wifi [%s]" % ", ".join(map(str, UsedWifi)))
    hw_file.write("\n Work [%s]" % ", ".join(map(str, dayWiFi)))
    hw_file.write("\n Home [%s]" % ", ".join(map(str, nightWiFi)))
    hw_file.write("\n Work {} and Home {} ".format(
        UsedWifi[idMaxD], UsedWifi[idMaxN]))
    hw_file.close()
...
def getListSeenWifi(db, device_id, UsedWifi, DEBUG=False):
    ...
    while True:
        ...
        if (
            datetime.strptime(
                ts2string(row[0]), "%Y-%m-%dT%H:%M:%S") > date_init
        ):  # on recupère les mesures depuis la date création de la table connectivity
            cpt = cpt + 1
            ts = row[0]  # timeslot
...
def getListConnectivity(db, device_id):
    ...
    cursor_connectivity.execute(
        """select timestamp_fire, type, ssid from connectivity c, measurements m where c.measurement_id=m.id and m.device_id='{}'""".format(
            (device_id)
        )
    )
    if cursor_connectivity.rowcount > 0:
        print(
            "la requete sur le Type de la table Connectivity a retourné 	{} 	lignes pour imei {}".format(
                cursor_connectivity.rowcount, device_id[0:4]
            )
        )
    else:
    ...
    if p != 0:
        ...
        for i in range(nbRnd):
            if random.random() < float(p):
                nb += int(1)
                continue
        if nb >= float(nbRnd * p * float(1 - window)) and nb <= float(
            nbRnd * p * float(1 + window)
        ):
            res = True
    return res
